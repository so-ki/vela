...
> 本文件是上一冻结 RC 的历史证据，只适用于文中明确列出的代码基线。当前开发分支的新增机制、迁移和本地测试见 [`DEVELOPMENT_CANDIDATE.md`](./DEVELOPMENT_CANDIDATE.md)；在新的 Draft PR 远端 CI 绿色并重新冻结前，不得把本文件的旧 CI 结果套用于新代码。
