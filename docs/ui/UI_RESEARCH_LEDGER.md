...
|---|---|---|---|---|---|
| [W3C WAI-ARIA APG — Tabs](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/) | tablist/tab/tabpanel 语义、键盘焦点与选中状态分离 | 法律研究三栏内容切换仍需可被键盘和辅助技术理解 | 仅在同一工作上下文的分面中使用 tabs；导航与流程步骤不用 tabs 冒充 | 使用 Vela 自有“证据工作台”标签、中文状态与审计焦点环 | 拒绝以纯视觉按钮组替代语义；拒绝把线性流程塞进 tabs |
...
| [Carbon — Tabs usage](https://carbondesignsystem.com/components/tabs/usage/) | tabs 用于内容分组，progress indicator 用于线性任务 | 防止“研究分面”和“交付过程”信息架构混淆 | 法律研究用分面 tabs；交付中心使用线性 evidence pipeline | 仅采纳用途区分，组件布局、尺度、色彩和动效全部自研 | 拒绝直接引入 Carbon 依赖或模仿 IBM 产品外观 |
